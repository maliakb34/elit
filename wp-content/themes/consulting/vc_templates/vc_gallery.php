<?php
$title = $source = $type = $onclick = $custom_links = $custom_links_target = $img_size = $external_img_size = $images = $custom_srcs = $el_class = $el_id = $interval = $css = $css_animation = ''; // phpcs:ignore
extract(
	shortcode_atts(
		array(
			'title'               => '',
...
if ( 'nivo' === $type ) {
	...
	$slides_wrap_end   = '</ul>';
} else if ( 'slick_slider' === $type || 'slick_slider_2' === $type ) {
...
foreach ( $images as $attach_id ) {
	...
	if ( isset( $post_thumbnail_nav['thumbnail'] ) ) {
