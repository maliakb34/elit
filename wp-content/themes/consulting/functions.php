...
define( 'STM_THEME_SLUG', 'consulting' );
...
add_filter( 'stm_theme_default_popup', 'get_stm_theme_default_popup' );
function get_stm_theme_default_popup() {
	return 'default';
}
...
add_filter( 'dashboard_hide_import_media', 'consulting_add_import_media' );
function consulting_add_import_media() {
	return true;
}
...
require_once CONSULTING_INC_PATH . '/header/theme-options.php';

/*Custom Title Box*/
require_once CONSULTING_INC_PATH . '/title-box/theme-options.php';
...
if ( ! function_exists( 'consulting_load_theme_scripts_and_styles' ) ) {
	function consulting_load_theme_scripts_and_styles() {
		if ( ! is_admin() ) {
			...
			wp_register_style( 'font-awesome', get_template_directory_uri() . '/assets/css/fontawesome/fontawesome.min.css', null, CONSULTING_THEME_VERSION, 'all' );
			wp_register_style( 'font-awesome-brands', get_template_directory_uri() . '/assets/css/fontawesome/brands.min.css', null, CONSULTING_THEME_VERSION, 'all' );
			wp_register_style( 'font-awesome-solid', get_template_directory_uri() . '/assets/css/fontawesome/solid.min.css', null, CONSULTING_THEME_VERSION, 'all' );
			wp_register_style( 'font-awesome-shims', get_template_directory_uri() . '/assets/css/fontawesome/v4-shims.min.css', null, CONSULTING_THEME_VERSION, 'all' );
			...
			wp_register_script(
				'Chart',
				get_template_directory_uri() . '/assets/js/Chart.min.js',
				array( 'jquery' ),
				'2.9.3',
				array(
					'strategy'  => 'defer',
			...
			wp_register_script(
				'jquery.cookie',
				get_template_directory_uri() . '/assets/js/jquery.cookie.min.js',
				array( 'jquery' ),
				CONSULTING_THEME_VERSION,
				array(
					'strategy'  => 'defer',
			...
			wp_register_script(
				'stocks-charts',
				get_template_directory_uri() . '/assets/js/stocks-indexes/stm-stocks-charts.js',
				array( 'jquery' ),
				CONSULTING_THEME_VERSION,
				array(
					'strategy'  => 'defer',
			...
			wp_register_script(
				'stocks-tables',
				get_template_directory_uri() . '/assets/js/stocks-indexes/stm-stocks-tables.js',
				array( 'jquery' ),
				CONSULTING_THEME_VERSION,
				array(
					'strategy'  => 'defer',
			...
			wp_register_script(
				'stocks-carousel',
				get_template_directory_uri() . '/assets/js/stocks-indexes/stm-stocks-carousel.js',
				array( 'jquery' ),
				CONSULTING_THEME_VERSION,
				array(
					'strategy'  => 'defer',
			...
			wp_register_script(
				'stocks-header',
				get_template_directory_uri() . '/assets/js/stocks-indexes/stm-stocks-header.js',
				array( 'jquery' ),
				CONSULTING_THEME_VERSION,
				array(
					'strategy'  => 'defer',
			...
			wp_register_script(
				'simplemarquee',
				get_template_directory_uri() . '/assets/js/stocks-indexes/jquery.simplemarquee.js',
				array( 'jquery' ),
				CONSULTING_THEME_VERSION,
				array(
					'strategy'  => 'defer',
			...
			wp_add_inline_style( 'consulting-global-styles', consulting_inline_styles() );

			$consulting_header_inline_css = consulting_header_css();
			if ( ! empty( $inline_css ) ) {
				wp_add_inline_style( 'consulting-global-styles', $consulting_header_inline_css );
			}

			$consulting_title_box_inline_css = consulting_title_box_css();
			if ( ! empty( $inline_css ) ) {
				wp_add_inline_style( 'consulting-global-styles', $consulting_title_box_inline_css );
			}
		}
	}
}
...
if ( ! function_exists( 'consulting_admin_styles' ) ) {
	function consulting_admin_styles() {
		...
		wp_register_style( 'consulting-font-awesome', get_template_directory_uri() . '/assets/css/fontawesome/fontawesome.css', null, CONSULTING_THEME_VERSION, 'all' );
		wp_register_style( 'consulting-font-awesome-brands', get_template_directory_uri() . '/assets/css/fontawesome/brands.css', null, CONSULTING_THEME_VERSION, 'all' );
		wp_register_style( 'consulting-font-awesome-solid', get_template_directory_uri() . '/assets/css/fontawesome/solid.css', null, CONSULTING_THEME_VERSION, 'all' );
		wp_register_style( 'consulting-font-awesome-shims', get_template_directory_uri() . '/assets/css/fontawesome/solid.css', null, CONSULTING_THEME_VERSION, 'all' );
...
if ( ! function_exists( 'consulting_ajaxurl' ) ) {
	function consulting_ajaxurl() {
		...
		$stm_ajax_add_pear_hb      = wp_create_nonce( 'stm_ajax_add_pear_hb' );
		?>
		<script type="text/javascript">
			...
			var stm_ajax_add_pear_hb 		 = '<?php echo esc_js( $stm_ajax_add_pear_hb ); ?>';
		</script>
		<?php
	}
}
...
if ( ! function_exists( 'consulting_body_class' ) ) {
	function consulting_body_class( $classes ) {
		...
		if ( ! is_404() && ! empty( $post ) ) {
			$enable_header_transparent = get_post_meta( $post->ID, 'enable_header_transparent', true );
			if ( 'on' === $enable_header_transparent || ( 'default' === $enable_header_transparent && consulting_theme_option( 'metabox_enable_header_transparent' ) ) ) {
				$classes[] = 'header_transparent';
			}

			if ( get_post_meta( $post->ID, 'header_inverse', true ) ) {
				$classes[] = 'header_inverse';
			}

			if ( ! empty( get_post_meta( $post->ID, 'title_box_bg_image', true ) ) ) {
		...
		if ( (bool) consulting_theme_option( 'header_wide', false ) ) {
			$classes[] = 'header_navigation_wide';
		}
		...
		$option_404 = consulting_theme_option( '404_page' );
		if ( is_404() && empty( $option_404 ) ) {
			$classes[] = 'default-404';
		}
...
require_once CONSULTING_INC_PATH . '/popups/main.php';
...
add_action( 'activate_instagram-feed/instagram-feed.php', 'consulting_disable_sbi_redirect' );
function consulting_disable_sbi_redirect() {
	$sbi_stasuses = get_option( 'sbi_statuses', array() );
	if ( ! isset( $sbi_stasuses['wizard_dismissed'] ) || false === $sbi_stasuses['wizard_dismissed'] ) {
		$sbi_stasuses['wizard_dismissed'] = true;
	}
	update_option( 'sbi_statuses', $sbi_stasuses );
}

function stm_ajax_add_pear_hb() {
	check_ajax_referer( 'stm_ajax_add_pear_hb', 'security' );
	update_option( 'add_pear_hb_status', sanitize_text_field( $_GET['add_pear_hb_status'] ) );
}

add_action( 'wp_ajax_stm_ajax_add_pear_hb', 'stm_ajax_add_pear_hb' );
add_action( 'wp_ajax_nopriv_stm_ajax_add_pear_hb', 'stm_ajax_add_pear_hb' );

/*Fixed contact form content p tag*/
add_filter( 'wpcf7_autop_or_not', '__return_false' );

function consulting_pear_hb_admin_notice() {
	if ( empty( get_option( 'add_pear_hb_status', '' ) ) ) {
		$init_data = array(
			'notice_type'          => 'animate-circle-notice pear_hb_message',
			'notice_logo'          => 'attent_circle.svg',
			'notice_title'         => esc_html__( 'Support for Pearl Header Builder', 'consulting' ),
			'notice_desc'          => esc_html__( 'Plugin will deprecate soon and related Header styles will be removed. We suggest to try Default Header Styles or Elementor Header & Footer builder. When different Header Builder selected, you will not be able to use Pearl Header Builder again and it will be disabled.', 'consulting' ),
			'notice_btn_one'       => '#',
			'notice_btn_one_title' => esc_html__( 'Agree', 'consulting' ),
			'notice_btn_one_class' => 'skip_pear_hb',
		);
...
add_action( 'admin_notices', 'consulting_pear_hb_admin_notice' );

add_action(
	'anp_popup_items',
	function () {
		if ( defined( 'STM_THEME_SLUG' ) ) {

			$themeInfo = new \ANP\ThemeInfoProvider( STM_THEME_SLUG );

			if ( $themeInfo->hasUpdate ) {
				$rateItem = new \ANP\Popup\Theme\ItemUpdateTheme(
					$themeInfo->thumbnail,
					$themeInfo->name . ' ' . $themeInfo->version . ' - ' . esc_html__( 'New update available!', 'consulting' ),
					$themeInfo->changelog,
					$themeInfo->updateLink,
					STM_CHANGELOG_URL,
					'theme_update'
				);

				\ANP\NotificationEnqueueControl::addMainItem( 'theme_update', $rateItem->createHtml() );
			}
		}
...
add_filter(
	...
	function () {
		$post_types_array       = array( 'stm-zoom-webinar', 'stm-zoom', 'stm_works', 'post', 'stm_event' );
		$post_type              = get_post_type( get_the_ID() );
		$post_content_elementor = get_post_meta( get_the_ID(), '_elementor_data', true );
		$post_content           = ! empty( $post_content_elementor ) ? $post_content_elementor : get_the_content();

		if ( in_array( $post_type, $post_types_array ) || false !== strpos( $post_content, 'addtoany' ) || false !== strpos( $post_content, 'stm_share_buttons' ) ) {
			return false;
		}
		return true;
	},
